    ...
    T: Hash,
{
    fn hash(&self) -> Option<u64> {
        if let Some(final_state) = &self.final_state {
            ...
            // pt.0: build a hash which uniquely identifies the state
            ...
            // Hash the cars positions and orientations
            final_state.cars().iter().for_each(|(pos, car)| {
                pos.hash(&mut hasher);
                car.orientation().hash(&mut hasher);
            });
...
fn step_until_seen<T: BoardValue>(
    ...
    // pt.0: find the obstacle first encountered in the direction provided
    ...
    loop {
        // Try to shift to the next position
        let next = match from.shift(direction, offset) {
            Some(pos) => pos,
            None => return View::new(direction, None, distance),
        };
        ...
        match board.get(next) {
            Some(Some(obstacle)) => {
                ...
                return View::new(
                    direction,
                    Some(NonZeroUsize::new(obstacle.get()).unwrap()),
                    distance,
                );
            }
            Some(None) => {
